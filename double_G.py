...
def k_fold_cross_validation(device, eyes_dataset, epochs, k_fold=5,
    ...
    kf = KFold(n_splits=k_fold, shuffle=True, random_state=1)  # init KFold
    # Iterations = 1
    ...
    # for fold, (train_idx, val_idx) in enumerate(skf.split(eyes_dataset, labels), 1):
        ...
        for e in range(1, epochs + 1):
            ...
            for batch_idx, (left_images, right_images, labels) in enumerate(train_iterator):
                ...
                loss = loss_fn(predictions, labels[:, 2:3])
                loss.backward()
                optimizer.step()
                ...
                train_metric.update(predictions, labels[:, 2:3])
            ...
            with torch.no_grad():
                ...
                for batch_idx, (left_images, right_images, labels) in enumerate(eval_iterator):
                    ...
                    valid_metric.update(predictions, labels[:, 2:3])
            ...
            if total_eval_loss < best_loss:
                ...
                best_epoch = e
                torch.save(model.state_dict(), f'./checkpoint/double_G_combine{fold}.pth')
...
if __name__ == '__main__':
    ...
    EPOCH = 500
    epoch = 0
    otuput_file = "double_G_combine.txt"
