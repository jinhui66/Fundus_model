...
from Dataset import DoubleTransformedSubset, DoubleEyesDataset
...
def k_fold_cross_validation(device, eyes_dataset, epochs, k_fold=5,
    ...
    # for fold, (train_idx, val_idx) in enumerate(skf.split(eyes_dataset, labels), 1):
        ...
        X = Count / X - 1
        print("各疾病损失权重:", X)
        # X = torch.tensor([6.9,15.0,17.1,20.4,39.1,23.9,3.7])
        ...
        # model = ResNet_stage1(BasicBlock, [2, 2, 2, 2], num_classes=2).to(device)
        model = DoubleResNet(num_classes=1).to(device)
        
        # model = EyeNet(num_classes=1).to(device)
        
        optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=1e-4, betas=(0.9, 0.98))
        ...
        for e in range(1, epochs + 1):
            ...
            for batch_idx, (left_images, right_images, labels) in enumerate(train_iterator):
                ...
                # print(f"image:{images[0][0][512][512]}")
                # print(f"labels:{labels}")
                optimizer.zero_grad()

                predictions = model(left_images, right_images)
                # print(predictions.shape, labels.shape)
                loss = loss_fn(predictions, labels[:,0:1])
                loss.backward()
                optimizer.step()
                ...
                # print("pos_predictions", pos_predictions)
                # print("labels:", labels[:,0])

                train_metric.update(predictions, labels[:,0:1])
            ...
            with torch.no_grad():
                ...
                for batch_idx, (left_images, right_images, labels) in enumerate(eval_iterator):
                    ...
                    valid_metric.update(predictions, labels[:,0:1])
            ...
            if average_accuracy > best_acc:
                ...
                best_epoch = e
                torch.save(model.state_dict(), f'./checkpoint/stage1_fold{fold}.pth')
...
if __name__ == '__main__':
    ...
    # 初始化自定义数据集
    dataset = DoubleEyesDataset(csv_file=args.csv_file_path,
