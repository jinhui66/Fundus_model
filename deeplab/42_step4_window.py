...
class MainWindow(QTabWidget):
    ...
    ***界面初始化***
    ...
    def detect_img(self):
        ...
        result, ratio = get_result(self.seg_model, img_path=self.img2predict)
        ...
        end_time = time.time()
        # 对 ratio结果进行判断
        result_txt = f"当前眼底图像的杯盘比为{ratio}, 安全阈值为{self.cd_th}\n"
        if ratio > 0.3:
            result_txt = result_txt + "您目前的指标正常，请继续保持\n友情提示：正常情况下，杯盘比大约在0.3左右，一般不会超过0.5, "
        else:
            result_txt = result_txt + "您目前的杯盘比超出安全阈值，请接收进一步的检查。\n温馨提示: 正常情况下，杯盘比大约在0.3左右，一般不会超过0.5, 建议进行进一步检查，杯盘比增大不一定是青光眼，还有一种可能是生理性杯盘比增大，一定要进行排青检查后才能确定。"
        self.img_info_edit3.setText(result_txt)
        time_str = str(round(float(end_time - start_time), 3)) + "s"
