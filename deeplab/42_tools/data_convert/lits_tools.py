    ...
    for k in range(z):
        ...
        if silce_seg.max() == 0:
            ...
            if (np.sum(silce_seg == 255) / total) > 0.015:
                ...
                # print(savepath_seg)
                silce_seg = np.array(silce_seg, np.uint8)
                silce_vol = np.array(silce_vol, np.uint8)
...
if __name__ == '__main__':
    ...
    path = 'G:/money/project_dz/4_april/700_ganzang/LiTs17/'
    savepath = 'G:/money/project_dz/4_april/700_ganzang/LiTs17/2d/'
    filenames = os.listdir(path + "segmentation") # 文件名称。
