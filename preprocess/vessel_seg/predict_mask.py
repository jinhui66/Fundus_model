...
def process_image(model, img_path, roi_mask_path, output_dir, device):
    ...
    with torch.no_grad():
        ...
        # 打印调整后的大小
        print(f"Resized mask size: {mask.size}")
...
def predict_mask(input_dir, roi_mask_dir, output_dir):
    ...
    # output_dir = "/data3/wangchangmiao/jinhui/eye/vessel_mask"  # 存放推理结果
    ...
    for img_filename in os.listdir(input_dir):
        ...
        # 检查是否为图像文件（这里假设图片扩展名为.tif, .png, .jpg, .jpeg等）
