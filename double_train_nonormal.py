...
def k_fold_cross_validation(device, eyes_dataset, epochs, k_fold=5,
    ...
    train_metric = Metric_Manager(num_classes=7)
    valid_metric = Metric_Manager(num_classes=7)
    ...
    # for fold, (train_idx, val_idx) in enumerate(skf.split(eyes_dataset, labels), 1):
        ...
        # model = DoubleResNet(num_classes=7).to(device)
        model = DoubleResNetShared(num_classes=7).to(device)
        ...
        for e in range(1, epochs + 1):
            ...
            for batch_idx, (left_images, right_images, labels) in enumerate(train_iterator):
                ...
                train_metric.update(predictions, labels[:, 1:])
            ...
            with torch.no_grad():
                ...
                for batch_idx, (left_images, right_images, labels) in enumerate(eval_iterator):
                    ...
                    valid_metric.update(predictions, labels[:, 1:])
            ...
            if total_eval_loss < best_loss:
                ...
                best_epoch = e
                torch.save(model.state_dict(), f'./checkpoint/share_fold{fold}.pth')
...
if __name__ == '__main__':
    ...
    EPOCH = 500
    epoch = 0
    otuput_file = "double_ietk256.txt"
