...
def k_fold_cross_validation(device, eyes_dataset, epochs, k_fold=5,
    ...
    # for fold, (train_idx, val_idx) in enumerate(skf.split(eyes_dataset, labels), 1):
        ...
        for e in range(1, epochs + 1):
            ...
            for batch_idx, (left_images, right_images, labels, text) in enumerate(train_iterator):
                left_images, right_images, labels = left_images.to(device), right_images.to(device), labels.to(device)
                text = text.to(device)
                optimizer.zero_grad()

                predictions = model(left_images, right_images, text)
            ...
            with torch.no_grad():
                ...
                for batch_idx, (left_images, right_images, labels, text) in enumerate(eval_iterator):
                    left_images, right_images, labels = left_images.to(device), right_images.to(device), labels.to(device)
                    text = text.to(device)
                    predictions = model(left_images, right_images, text)
            ...
            if total_score > best_score:
                ...
                best_epoch = e
                torch.save(model.state_dict(), f'./checkpoint/multimodal{fold}.pth')
...
if __name__ == '__main__':
    ...
    EPOCH = 200
    epoch = 0
    otuput_file = "double_pretrained_multimodal3.txt"
