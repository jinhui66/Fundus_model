...
def k_fold_cross_validation(device, eyes_dataset, epochs, k_fold=5,
    ...
    train_metric = Metric_Manager(num_classes=8)
    valid_metric = Metric_Manager(num_classes=8)
    ...
    # for fold, (train_idx, val_idx) in enumerate(skf.split(eyes_dataset, labels), 1):
        ...
        X = Count / X - 1
        print("各疾病损失权重:", X)
        # X = torch.tensor([1.4978,  4.6430, 12.7381, 12.4971, 16.7538, 34.5077, 14.7007,  2.1147])
        ...
        model = DoubleResNet(num_classes=8).to(device)
        ...
        for e in range(1, epochs + 1):
            ...
            for batch_idx, (left_images, right_images, labels) in enumerate(train_iterator):
                ...
                pos_predictions = to0_1(pos_predictions)
                
                # print("predictions:", predictions)
                # print("pos_predictions", pos_predictions)
                # print("labels:", labels[:,0])

                train_metric.update(pos_predictions, labels)
            ...
            with torch.no_grad():
                ...
                for batch_idx, (left_images, right_images, labels) in enumerate(eval_iterator):
                    ...
                    valid_metric.update(pos_predictions, labels)
            ...
            if average_accuracy > best_acc:
                ...
                best_epoch = e
                torch.save(model.state_dict(), f'./checkpoint/fold{fold}.pth')
