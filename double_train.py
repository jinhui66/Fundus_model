...
def k_fold_cross_validation(device, eyes_dataset, epochs, k_fold=5,
    ...
    # for fold, (train_idx, val_idx) in enumerate(skf.split(eyes_dataset, labels), 1):
        ...
        # 应用转换
        train_dataset = CombineTransformedSubset(k_train_fold, transform=None)
        val_dataset = CombineTransformedSubset(k_test_fold, transform=None)
        ...
        model = DoubleResNetShared(num_classes=7, input_channels=7).to(device)
